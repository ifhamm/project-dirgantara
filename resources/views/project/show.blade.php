        ...
        @foreach($phaseConfig as $type => $cfg)
            ...
            <li class="nav-item">
                ...
                >
                    ...
                    @if($phase)
                        <span class="ms-2 badge rounded-pill" style="background: {{ $cfg['color'] }}20; color: {{ $cfg['color'] }}; font-size: 0.7rem;">
                        ...
                        <div class="task-group-block mb-3">
                            <div class="task-group-header d-flex align-items-center justify-content-between px-3 py-2 rounded-top"
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    @if($taskGroup->no)
                                        <span class="badge fw-semibold" style="background: {{ $cfg['color'] }}; font-size: 0.72rem;">
                                            {{ $taskGroup->no }}
                                        </span>
                                    @endif
                                ...
                                <div class="task-row px-3 py-2 {{ ! $loop->last ? 'border-bottom' : '' }}">
                                    <div class="d-flex align-items-start justify-content-between gap-2">
                                        <div class="flex-grow-1">
                                            <div class="d-flex align-items-center gap-2 mb-1">
                                                @if($task->no)
                                                    <span class="text-muted small fw-semibold" style="min-width: 2rem;">{{ $task->no }}</span>
                                                @endif
                                            ...
                                            <div class="d-flex flex-wrap gap-2 mt-1 ms-{{ $task->no ? '4' : '0' }}">
                                                @foreach($task->mwsParts as $mws)
                                                <a href="{{ route('mws.show', $mws->id) }}"
                                                   class="mws-chip"
                                                   title="{{ $mws->name }}">
                                                    <i class="fas fa-file-alt me-1" style="font-size: 0.65rem;"></i>
                                                    {{ $mws->part_id ?? $mws->name }}
                                                    <span class="mws-status mws-status--{{ $mws->status ?? 'open' }}">
                                        ...
                                        <div class="text-end" style="min-width: 50px;">
                                            <span class="small fw-semibold" style="color: {{ $cfg['color'] }};">
                                                {{ $taskProgress }}%
                                            </span>
                                        </div>
                                    </div>
...
<style>
    ...
    .mws-status--open       { background: #e3f2fd; color: #1565c0; }
    .mws-status--inprogress { background: #fff8e1; color: #e65100; }
