                ...
                <div class="card-body">
                        ...
                        @foreach ($infoFields as $field)
                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                <div class="bg-light p-3 rounded-2 h-100 border">
                                    <small
                                        class="d-block text-muted text-uppercase fw-bold mb-2">{{ $field['label'] }}</small>
                    ...
                    <form id="mws-info-edit" class="d-none" onsubmit="saveMwsInfo(event)">
                        ...
                        <div class="row">
                            @php
                                $editFields = [
                                    ...
                                    ['name' => 'shopArea', 'label' => 'Shop Area', 'value' => $mwsPart->shop_area],
                            ...
                            @foreach ($editFields as $f)
                                <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                                    ...
                                    <input type="{{ $f['type'] ?? 'text' }}" name="{{ $f['name'] }}"
                                        value="{{ $f['value'] }}" class="form-control form-control-sm">
                                </div>
                            @endforeach
                        </div>
                        <div class="d-flex gap-2 mt-3">
                            ...
                            <button type="button" onclick="toggleEditMwsInfo(false)" class="btn btn-sm btn-secondary"><i
                        ...
                        <div class="col-12 col-sm-6 col-md-4">
                            <div class="bg-light p-3 rounded">
                                ...
                                @php
                                    ...
                                    $strippingColor =
                                        $strippingPct > 75 ? 'success' : ($strippingPct > 40 ? 'warning' : 'danger');
                                @endphp
                                <div class="progress mt-2">
                                    <div class="progress-bar bg-{{ $strippingColor }}"
                ...
                <div id="attachment-section" class="card mb-4 d-none">
                    ...
                    <div class="card-body">
                        <div class="d-flex gap-2 mb-3">
                            <input type="file" id="mws-attachment-input" multiple class="d-none"
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    @can('update', $mwsPart)
                                        <button onclick="deleteMwsAttachment('{{ $mwsPart->id }}', '{{ $att['public_id'] }}')"
                                            class="btn btn-sm btn-link text-danger" title="Hapus">
                ...
                <div class="card-header bg-light d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        @can('update', $mwsPart)
                            ...
                            <button onclick="addFirstStep('{{ $mwsPart->id }}')" class="btn btn-sm btn-success"
                                ...
                                <tbody id="consumables-tbody">
                                    @forelse($mwsPart->consumables as $consumable)
                                        <tr id="consumable-row-{{ $consumable->id }}">
                                            <td><span id="cons-name-{{ $consumable->id }}">{{ $consumable->name }}</span>
                                            </td>
                                            <td><span
                                                    id="cons-ident-{{ $consumable->id }}">{{ $consumable->identification ?? '-' }}</span>
                                            </td>
                                            <td class="text-center"><span
                                                    id="cons-qty-{{ $consumable->id }}">{{ $consumable->quantity }}</span>
                                            </td>
                                            @can('update', $mwsPart)
                                                <td class="text-center" id="consumable-actions-{{ $consumable->id }}">
                                                    <button type="button"
                                                        onclick="editConsumable('{{ $mwsPart->id }}', {{ $consumable->id }})"
                                                        class="btn btn-outline-primary btn-sm me-1"
                                                        title="Edit">Edit</button>
                                                    <button type="button"
                                                        onclick="deleteConsumable('{{ $mwsPart->id }}', {{ $consumable->id }})"
                                                        class="btn btn-outline-danger btn-sm" title="Hapus">Hapus</button>
                                                </td>
                                            @endcan
                                        </tr>
                                    @empty
                                        <tr id="consumables-empty-row">
                                            <td colspan="4" class="text-center text-muted py-3">Belum ada consumable.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                    ...
                                    <tfoot>
                                        <tr id="consumable-add-trigger" class="table-light">
                                            <td colspan="4" class="text-center py-2">
                                                <button type="button" onclick="toggleConsumableAdd(true)"
                                        ...
                                        <tr id="consumable-add-row" class="table-light d-none">
                                            <td><input type="text" id="new-cons-name" class="form-control form-control-sm"
                                                    placeholder="Nama consumable..."></td>
                                            <td><input type="text" id="new-cons-ident"
                                                    class="form-control form-control-sm"
                                                    placeholder="Identification / References..."></td>
                                            <td><input type="text" id="new-cons-qty" class="form-control form-control-sm"
                                                    placeholder="AR"></td>
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-2">
                                                    <button type="button" onclick="addConsumable('{{ $mwsPart->id }}')"
                                                        class="btn btn-success btn-sm px-3">Save</button>
                                                    <button type="button" onclick="toggleConsumableAdd(false)"
                                                        class="btn btn-secondary btn-sm px-3">Cancel</button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tfoot>
                        ...
                        <table class="worksheet-table table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    @endcan
                                    <th rowspan="2" class="col-no text-center p-3 fw-bold text-secondary align-middle">
                                        NO</th>
                                    <th rowspan="2"
                                        class="col-desc text-start p-3 fw-bold text-secondary align-middle">
                                        DESCRIPTION</th>
                                    <th colspan="2" class="text-center p-3 fw-bold text-secondary">PLAN</th>
                                    <th colspan="2" class="text-center p-3 fw-bold text-secondary">ACTUAL</th>
                                    <th rowspan="2"
                                ...
                                @forelse($mwsPart->steps->sortBy('no') as $step)
                                    ...
                                    <tr id="step-row-{{ $step->no }}" data-id="{{ $step->id }}"
                                                ...
                                                @foreach ($step->subSteps as $sub)
                                                    <div id="substep-item-{{ $sub->id }}"
                                                        ...
                                                        <div class="small text-dark d-flex gap-2 lh-base">
                                                            <span
                                                                class="fw-semibold text-secondary">{{ $sub->label }}.</span>
                                                            <span
                                                                id="substep-text-{{ $sub->id }}">{{ $sub->description }}</span>
                                                        </div>
                                                        @can('update', $mwsPart)
                                            ...
                                            @can('update', $mwsPart)
                                                ...
                                                <div class="pt-3 border-top">
                                                    ...
                                                    <button onclick="addDetail('{{ $mwsPart->id }}', {{ $step->no }})"
                                                        class="btn btn-outline-primary btn-sm mt-3 px-3">Tambah
                                                        Catatan</button>
                                                </div>
                                            @endcan
                                        </td>
                                        ...
                                        <td class="col-plan-man align-top">
                                            @can('update', $mwsPart)
                                                ...
                                                <div id="plan-man-view-{{ $step->no }}"
                                                    ...
                                                    <span id="plan-man-text-{{ $step->no }}"
                                                        class="small text-secondary">
                                                        {{ $step->plan_man ?? 'N/A' }}
                                                    </span>
                                                    <button onclick="togglePlanEdit({{ $step->no }}, 'man', true)"
                                                        class="btn btn-link btn-sm p-1" title="Edit Plan Man">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                </div>
                                                {{-- EDIT: tersembunyi default --}}
                                                <div id="plan-man-edit-{{ $step->no }}" style="display:none;">
                                                    <input type="text" id="plan-man-input-{{ $step->no }}"
                                                        value="{{ $step->plan_man }}"
                                                        class="form-control form-control-sm mb-2" placeholder="Contoh: 2">
                                                    <button
                                            ...
                                            @else
                                                <p class="text-center text-muted small">{{ $step->plan_man ?? 'N/A' }}</p>
                                            @endcan
                                        </td>
                                        ...
                                        <td class="col-plan-hrs align-top">
                                            @can('update', $mwsPart)
                                                <div id="plan-hours-view-{{ $step->no }}"
                                                    ...
                                                    <button onclick="togglePlanEdit({{ $step->no }}, 'hours', true)"
                                                        class="btn btn-link btn-sm p-1" title="Edit Plan Hours">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                </div>
                                            ...
                                            @else
                                                <p class="text-center text-muted small">{{ $step->plan_hours ?? 'N/A' }}</p>
                                            @endcan
                                        </td>
                                        ...
                                        <td class="col-tech align-top">
                                            ...
                                            @else
                                                <div class="d-flex flex-column align-items-center justify-content-center"
                                                    style="min-height: 40px; gap: 0.5rem;">
                                                ...
                                                @if ($isFinal)
                                                    ...
                                                    <div class="flex flex-col space-y-2 p-1"
                                                        ...
                                                        <select id="status-s-us-select-{{ $step->no }}"
                                                            ...
                                                            <option value="RAI">RAI (Release Authorize Inspector)
                                                            </option>
                                        ...
                                        <td class="col-action text-center align-top">
                                            ...
                                            @elseif($isMechanic)
                                                <div class="flex flex-col items-center space-y-1">
                                                    ...
                                                    @if ($step->status === 'pending' && !$techApproved)
                                                        <span class="text-xs text-gray-400">Klik Approve untuk
                                                            melanjutkan</span>
                                                    @endif
                                                </div>
                                            @elseif(auth()->user()->hasRole('quality1') || auth()->user()->hasRole('quality2'))
                                                <div class="flex flex-col items-center space-y-1">
                                                    <span class="text-xs text-gray-500">{{ $step->status }}</span>
                                                    @if ($stepInProgress)
                                                        <span class="text-xs text-gray-400">Klik Finish untuk
                                                            menyelesaikan</span>
                                                    @endif
                                                </div>
                                        ...
                                        <td class="col-status text-center align-middle">
                                            @if ($stepCompleted)
                                                <i class="fas fa-check-circle text-blue-500 text-xl"
                                                    title="Completed"></i>
                    ...
                    <div class="col-lg-6 mb-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                {{-- Prepared By --}}
                                @php
                                    $preparedBy = $mwsPart->prepared_by;
                                    $preparedDate = $mwsPart->prepared_date;
                                @endphp

                                <div
                                    class="p-3 mb-3 rounded border {{ $preparedBy ? 'border-success bg-light' : 'border-light bg-light' }}">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            ...
                                            @if ($preparedBy)
                                                <div class="d-flex align-items-center mb-1">
                                                    ...
                                                    <p class="mb-0 fw-medium small">{{ $preparedBy }}</p>
                                                </div>
                                                <small class="text-muted">
                                                    {{ $preparedDate ? \Carbon\Carbon::parse($preparedDate)->format('d/m/Y H:i') : '' }}
                                                </small>
                                            @else
                                                <p class="mb-0 text-muted small">Menunggu Approved...</p>
                                            @endif
                                        </div>
                                        @if (!$preparedBy && in_array(auth()->user()->role ?? '', ['admin', 'superadmin']))
                                        ...
                                        @elseif($preparedBy)
                                            <div class="d-flex align-items-center ms-2">
                                                ...
                                                @if (in_array(auth()->user()->role ?? '', ['admin', 'superadmin']))
                                                    <button
                                                        onclick="cancelSignature('{{ $mwsPart->id }}', 'prepared', 'Batalkan tanda tangan Prepared By?')"
                                                        class="btn btn-link btn-sm text-danger ms-2">Batal</button>
                                                @endif
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div
                                    class="p-3 mb-3 rounded border {{ $approvedBy ? 'border-success bg-light' : 'border-light bg-light' }}">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            ...
                                            @if ($approvedBy)
                                                <div class="d-flex align-items-center mb-1">
                                                    <i class="fas fa-user-check text-success me-2"></i>
                                                    <p class="mb-0 fw-medium small">{{ $approvedBy }}</p>
                                                </div>
                                                <small class="text-muted">
                                                    {{ $approvedAt ? \Carbon\Carbon::parse($approvedAt)->format('d/m/Y H:i') : '' }}
                                                </small>
                                            @else
                                                <p class="mb-0 text-muted small">Menunggu Approved...</p>
                                            @endif
                                        </div>
                                        @if (!$approvedBy && in_array(auth()->user()->role ?? '', ['admin', 'superadmin']))
                                        ...
                                        @elseif($approvedBy)
                                            <div class="d-flex align-items-center ms-2">
                                                ...
                                                @if (in_array(auth()->user()->role ?? '', ['admin', 'superadmin']))
                                                    <button
                                                        onclick="cancelSignature('{{ $mwsPart->id }}', 'approved', 'Batalkan tanda tangan Approved By?')"
                                                        class="btn btn-link btn-sm text-danger ms-2">Batal</button>
                                                @endif
                                            </div>
                                        @endif
                                ...
                                {{-- Verified By --}}
                                @php
                                    $verifiedBy = $mwsPart->verified_by;
                                    $verifiedAt = $mwsPart->verified_at;
                                @endphp

                                <div
                                    class="p-3 rounded border {{ $verifiedBy ? 'border-success bg-light' : 'border-light bg-light' }}">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            ...
                                            @if ($verifiedBy)
                                                <div class="d-flex align-items-center mb-1">
                                                    <i class="fas fa-user-check text-success me-2"></i>
                                                    <p class="mb-0 fw-medium small">{{ $verifiedBy }}</p>
                                                </div>
                                                <small class="text-muted">
                                        ...
                                        @elseif($verifiedBy)
                                            <div class="d-flex align-items-center ms-2">
                                                ...
                                                @if (in_array(auth()->user()->role ?? '', ['admin', 'superadmin']))
                                                    <button
                                                        onclick="cancelSignature('{{ $mwsPart->id }}', 'verified', 'Batalkan tanda tangan Verified By?')"
                                                        class="btn btn-link btn-sm text-danger ms-2">Batal</button>
                                                @endif
                                            </div>
                                        @endif
