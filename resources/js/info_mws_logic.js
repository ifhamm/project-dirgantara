...
async function addFirstStep(mwsPartId) {
    ...
    try {
        await apiFetch(`/mws/${mwsPartId}/steps`, "POST", { description: desc });
...
async function insertStepAfter(mwsPartId, stepNo) {
    const desc = prompt("Deskripsi step baru (akan disisipkan setelah step ini):");
    if (!desc) return;

    try {
        await apiFetch(`/mws/${mwsPartId}/steps/${stepNo}/insert-after`, "POST", {
            description: desc,
        });
...
function toggleCautionEdit(stepNo, show) {
    document.getElementById(`caution-edit-${stepNo}`)?.classList.toggle("hidden", !show);
}

function toggleNoteEdit(stepNo, show) {
    document.getElementById(`note-edit-${stepNo}`)?.classList.toggle("hidden", !show);
}

async function saveCaution(mwsPartId, stepNo) {
    const caution = document.getElementById(`caution-input-${stepNo}`)?.value ?? "";
...
function editConsumable(consumableId) {
    ...
    fields.forEach(({ key, elId }) => {
        ...
        input.className   = "w-full border rounded px-1 py-0.5 text-sm focus:ring-1 focus:ring-blue-400";
...
async function saveConsumable(mwsPartId, consumableId) {
    const name  = document.getElementById(`cons-name-input-${consumableId}`)?.value.trim();
    const ident = document.getElementById(`cons-ident-input-${consumableId}`)?.value.trim();
    const qty   = document.getElementById(`cons-qty-input-${consumableId}`)?.value.trim();
...
async function deleteConsumable(mwsPartId, consumableId) {
    ...
    try {
        await apiFetch(`/mws/${mwsPartId}/consumables/${consumableId}`, "DELETE");
...
async function assignMechanicToStep(mwsPartId, stepNo) {
    ...
    try {
        await apiFetch(`/mws/${mwsPartId}/steps/${stepNo}/assign-mechanic`, "POST", {
            nik,
        });
...
async function finishFinalInspection(mwsPartId, stepNo) {
    ...
    try {
        await apiFetch(`/mws/${mwsPartId}/steps/${stepNo}/finish-final`, "POST", {
            status_s_us: status,
        });
...
async function uploadMwsAttachment(mwsPartId) {
    ...
    if (!input?.files.length) return showToast("Pilih file terlebih dahulu.", "error");
...
async function uploadStepAttachment(mwsPartId, stepNo) {
    ...
    try {
        const res = await fetch(`/mws/${mwsPartId}/steps/${stepNo}/attachments`, {
            method: "POST",
            headers: { "X-CSRF-TOKEN": csrfToken },
            body: formData,
        });
